...
class Tenant:
    ...
    def step(self, action: int) -> None:
        ...
        # --- Movement ---
        if act in (Action.MOVE_FORWARD, Action.MOVE_BACKWARD,
    ...
    def _move(self, act: Action) -> None:
        ...
        if act == Action.MOVE_FORWARD:
            vx, vy = sin_y * MOVE_SPEED, cos_y * MOVE_SPEED
        elif act == Action.MOVE_BACKWARD:
            vx, vy = -sin_y * MOVE_SPEED, -cos_y * MOVE_SPEED
        elif act == Action.MOVE_LEFT:
            vx, vy = -cos_y * MOVE_SPEED, sin_y * MOVE_SPEED
        else:  # MOVE_RIGHT
            vx, vy = cos_y * MOVE_SPEED, -sin_y * MOVE_SPEED

        lin_vel, _ = pb.getBaseVelocity(
    ...
    def _put_down(self) -> None:
        ...
        pos = self.get_position()
        surface = self._nearest_reachable(
            category=None, exclude_held=False
        )
